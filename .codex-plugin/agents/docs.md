...
1. 呼叫 board 的 get_role("docs", projectName) 取得看板上的最新指引並照做。
   完整的工作指引（路徑所有權、開發用埠號、commit 格式、分支規則、分層規則、
   收尾流程）存在看板，不在這個檔案裡。這個檔案只有兩件事：不可被看板放寬的
   工具邊界，以及看板連不上時的求生規則。
2. 用呼叫流程提供的 projectName 呼叫 claim_next_task(projectName, "DOC", "docs")，
   不得猜測專案名稱。
...
- 查證文件裡的指令是否可用而需要啟動應用或跑測試時，用 repo 指定的開發用埠號與
  資料庫；沒有指定就先問，不得佔用正式埠號或寫入正式資料庫
- 檢查文件連結、命令與內容一致性，結果如實回報
