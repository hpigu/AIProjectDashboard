...
**已發布最新版**：v3.1.0（2026-08-05，僅單一
`ai-project-board-backend-3.1.0.jar` ＋ `.sha256`，早於本批三平台契約；四平台
asset──Linux x64／macOS arm64／macOS x64 JAR、Windows ZIP──與集中式
`SHA256SUMS.txt` 目前**尚不存在**，見下方「未解決的產品發布缺口」）｜
**下一批**：v3.2.0
